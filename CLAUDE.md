...
- **`device.py`** — `GoveeDevice`: Represents a single Govee light. Holds state (on/off, brightness, color, temperature), exposes the model's Kelvin range as `temperature_range` (falling back to the default range when the device has no capabilities) and exposes async control methods that delegate back to the controller.
- **`message.py`** — Message protocol. `GoveeMessage` is the base class; subclasses handle specific commands (`ScanMessage`, `OnOffMessage`, `ColorMessage`, `BrightnessMessage`, etc.). `PtRealMessage` handles binary commands with XOR checksum encoding via base64. `MessageResponseFactory` parses incoming JSON messages into typed response objects. `ColorMessage` clamps white temperatures to the range it is given, falling back to `DEFAULT_TEMPERATURE_RANGE` imported from `light_capabilities` — the message layer keeps no Kelvin constants of its own.
- **`light_capabilities.py`** — Device capability registry. `GoveeLightCapabilities` defines what features a device model supports (RGB, color temperature, brightness, segments, scenes) and its Kelvin `temperature_range` (`GoveeTemperatureRange`, defaulting to `DEFAULT_TEMPERATURE_RANGE` = 2000-9000 K — the single source of truth for the default range, also used by `ColorMessage`). `GOVEE_LIGHT_CAPABILITIES` is the dict mapping SKU strings to capabilities. New device support is added here.
- **`device_registry.py`** — `DeviceRegistry`: Internal store of discovered devices and a queue for manually-added device IPs.

## Adding Support for New Devices

To add a new Govee device model, add an entry to `GOVEE_LIGHT_CAPABILITIES` in `src/govee_local_api/light_capabilities.py`. Use `create_with_capabilities(rgb, temperature, brightness, segments, scenes, temperature_range=None)` or reference `BASIC_CAPABILITIES` for standard devices. The segments parameter is the number of individually controllable LED segments. Pass `temperature_range=(min_kelvin, max_kelvin)` only for models whose white range differs from the 2000-9000 K default; `ColorMessage` clamps to the device's range.
